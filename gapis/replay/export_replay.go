...
// TODO(hysw): Exporter.Replay have a different behaivor than Manager.Replay.
// It returns nil instead of the postback, since replay will not actually
...
func (m *exportManager) ForExport() bool { return true }

func (m *exportManager) Export(ctx context.Context) (*gapir.Payload, error) {
	...
	builderBuildTimer.Time(func() { payload, _, _, err = b.Build(ctx) })
	if err != nil {
...
// NOTE: Only works for query that does not require result to be passed back.
func (m *exportManager) Replay(
	...
	out := make(chan res, 1)
	m.requests = append(m.requests, RequestAndResult{
		Request: req,
